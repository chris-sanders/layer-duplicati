...
from charms.reactive import when_not, set_state, when_all, when, endpoint_from_name
...
def install_duplicati():
    ...
    fetch.add_source(
        "deb http://download.mono-project.com/repo/ubuntu stable-{series} main",
    ...
    try:
        os.mkdir(dh.data_dir)
    except OSError as e:
        if e.errno == 17:
            pass  # Continue if already exists
    host.chownr(dh.data_dir, "duplicati", "duplicati", chowntopdir=True)
...
def configure_reverseproxy():
    ...
    proxy_info = {
        "urlbase": dh.charm_config["proxy-url"],
